# Wheel of Python
# Below is the game logic for the rest of the “Wheel of Python” game. We have implemented most of the game logic.
# Start by carefully reading this code and double checking that it all makes sense. Then, paste your code from the
# previous code window in the correct places below.
# Note 1: we added the following code to ensure that the Python interpreter gives our game time to run: import sys
# sys.setExecutionLimit(600000)
# sys.setExecutionLimit(ms) says that we should be able to run our program for ms milliseconds before it gets stopped
# automatically.
...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category, obscuredPhrase, guessed):
        return input(f"""{self.name} has ${self.prizeMoney}
Category: {category}
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        if self.prizeMoney < VOWEL_COST:
            lst = [l for l in LETTERS if l not in guessed]
            lst2 = [l for l in lst if l not in VOWELS]
            return lst2
        else:
            lst = [l for l in LETTERS if l not in guessed]
            return lst
